...
setwd("/path/to/home/TRS_project/MR/instruments/clumped_instruments_reverse/")
...
# We can format the whole PD GWAS now for later as it is always the exposure here
...
instruments <- fread(paste0("PD_instruments_list.txt"), header = F)

PD_GWAS_formatted_instruments <- PD_GWAS_formatted %>%
  filter(SNP %in% instruments$V1)


F_stats <- PD_GWAS_formatted_instruments$beta.exposure^2/PD_GWAS_formatted_instruments$se.exposure^2
...
for (i in need_proxy) {
  ...
  outcome_name <- str_remove(i, "_instruments_list_needs_proxies.txt")
  outcome_name <- str_remove(outcome_name, "PD_")
...
if (nrow(need_proxy_instruments) == 0) {
  ...
  outcome_file <- fread(
    paste0("/path/to/home/TRS_project/MR/input/exposures/",
           outcome_name, "_alleles_checked_1KG.txt")
  ) %>%
    filter(snp %in% instruments$V1)

  outcome_file <- outcome_file[!duplicated(outcome_file$snp), ]
  
  outcome_file$Phenotype <- outcome_name
  
  outcome_file <- as.data.frame(outcome_file)
  
  outcome_formatted <- format_data(
    outcome_file,
    type = "outcome",
    snps = PD_GWAS_formatted_instruments$SNP,
    header = TRUE,
    snp_col = "snp",
    beta_col = "b",
    se_col = "se",
  ...
  harmonised_data <- harmonise_data(
    PD_GWAS_formatted_instruments,
    outcome_formatted,
    action = 2
  )
  
  # Save
  write.table(harmonised_data, file = paste0("/path/to/home/TRS_project/MR/output/harmonised_reverse/",exposure_name,"_PD_harmonised.txt"), sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
  ...
  outcome_file <- as.data.frame(fread(
    paste0("/path/to/home/TRS_project/MR/input/exposures/",
           outcome_name, "_alleles_checked_1KG.txt")))
  ...
  matched_snps_list <- fread(paste0("PD_",outcome_name, "_instruments_list_matched.txt"), header = F)
  matched_snps <- outcome_formatted %>% filter(SNP %in% matched_snps_list$V1)
  ...
  system("mv combined_query_snp_list_grch37.txt reverse_MR_proxy_snps.txt")
  
  outcome_data_w_proxies <- munge_proxies("reverse_MR_proxy_snps.txt", outcome_formatted, matched_snps)
  tail(outcome_data_w_proxies)
  ...
  # Save
  write.table(harmonised_data, file = paste0("/path/to/home/TRS_project/MR/output/harmonised_reverse/PD_",outcome_name,"_harmonised.txt"), sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
